...
## [`6.18.3.dev0` (unreleased)](https://github.com/kdeldycke/repomatic/compare/v6.18.2...main)
...
- Switch the `compile-binaries` job to Nuitka's `--main-entry-point=<bin>=<module>:<callable>` flag and `--mode=onefile`. The matrix entry now exposes `main_entry_point` instead of `module_path`, so `nuitka_matrix` no longer probes the filesystem to derive a module path, no longer special-cases `__main__.py` packages, and no longer injects `--python-flag=-m`: Nuitka generates the wrapper main program directly from the entry-point notation. `--output-folder-name` is also passed (matching the binary name) because without a positional script file, Nuitka has no `_main_module` to derive the build folder name from and `OutputDirectories._getResultBaseName` crashes with `AttributeError: 'NoneType' object has no attribute 'getOutputFilename'`. `--mode=onefile` is the documented spelling since Nuitka 4.0 (`--onefile` is hidden but still accepted).
