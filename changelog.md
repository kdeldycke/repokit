...
- Expose `is_version_bump_branch` as a new `repomatic metadata` output: it returns `True` when `GITHUB_HEAD_REF` matches one of the `VERSION_BUMP_BRANCHES` members (`prepare-release`, `major-version-increment`, `minor-version-increment`), and `False` for push/schedule/workflow_dispatch events. `lint.yaml` and `labels.yaml` consume the new field: their `metadata` job no longer needs its own `github.head_ref != 'prepare-release'` guard, and every dependent job now gates on `!fromJSON(needs.metadata.outputs.metadata).is_version_bump_branch`. This collapses eight scattered per-job branch-name checks into a single source-of-truth lookup, and extends the skip semantics to cover the version-increment branches via downstream `workflow_call` invocations (where the workflow-level `branches-ignore` filter does not apply).
