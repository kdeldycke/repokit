...
- Extend the same `metadata` head-ref gate to `tests.yaml`. The trigger-level `pull_request.branches-ignore` filter matches the PR's *base* branch (always `main`) and does not actually exclude version-bump PRs, which is why their test matrices were still running daily. The new `github.head_ref` check is the real skip mechanism; `branches-ignore` is kept as documentation of intent. `test_metadata_gate_skips_version_bump_branches` now covers `tests.yaml` alongside `lint.yaml` and `labels.yaml`.
