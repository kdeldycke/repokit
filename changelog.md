...
- Extend the `pull_request` `branches-ignore` filter in `tests.yaml`, `lint.yaml`, and `labels.yaml` to skip `major-version-increment` and `minor-version-increment` PRs in addition to `prepare-release`. These automated drafts only touch version strings and `uv.lock`, so the code is byte-identical to `main` and the test/lint matrices yield no signal. Lockfile-induced regressions are still independently caught by `sync-uv-lock` autofix PRs against `main`.
- Introduce the `VERSION_BUMP_BRANCHES` constant in `repomatic.binary` alongside `SKIP_BINARY_BUILD_BRANCHES`. Two new conformance tests pin the constant against both ends of its contract: `test_version_bump_branches_ignored` asserts the heavy PR-triggered workflows (`tests.yaml`, `lint.yaml`, `labels.yaml`) keep every member of the set in their `pull_request.branches-ignore` list, and `test_version_bump_branches_match_changelog_workflow` asserts the constant matches the actual branches that `changelog.yaml`'s `peter-evans/create-pull-request` steps produce (the `prepare-release` literal plus the `${{ matrix.part }}-version-increment` expansion over the `bump-version` job's matrix). The two sets `SKIP_BINARY_BUILD_BRANCHES` and `VERSION_BUMP_BRANCHES` are documented and asserted disjoint: autofix branches keep their binary-neutral semantics, while version-bump branches *do* change the baked-in version string but should still skip PR-time CI.
- Drop the redundant `github.head_ref != 'prepare-release'` clause from the two Codecov upload steps in `tests.yaml`. The workflow's `pull_request.branches-ignore` already prevents the workflow from triggering on a `prepare-release` PR, and `tests.yaml` has no `workflow_call` entry point that could bypass that filter, so the check was dead code. The remaining `sync-repomatic` guard stays in place (`sync-repomatic` is not in `branches-ignore`).
- Expose `is_version_bump_branch` as a new `repomatic metadata` output: it returns `True` when `GITHUB_HEAD_REF` matches one of the `VERSION_BUMP_BRANCHES` members (`prepare-release`, `major-version-increment`, `minor-version-increment`), and `False` for push/schedule/workflow_dispatch events. Useful for callers (downstream workflows, CLI consumers) that need to branch on the current event's PR origin.
- Gate the `metadata` pre-job in `lint.yaml` and `labels.yaml` directly on `!contains(fromJSON('[...]'), github.head_ref)` over the `VERSION_BUMP_BRANCHES` list. When the gate skips, GitHub Actions propagates the skip to every dependent job via `needs: metadata`, so version-bump PRs cost zero runner minutes in both workflows (including downstream `workflow_call` invocations that bypass the `pull_request.branches-ignore` trigger filter). Eight per-job `github.head_ref != 'prepare-release'` guards in `lint.yaml` collapse to a single gate on the `metadata` job; the labellers in `labels.yaml` lose their now-redundant version-bump check. A new conformance test (`test_metadata_gate_skips_version_bump_branches`) parses the embedded JSON array and asserts it stays in sync with the Python constant.
- Extend the same `metadata` pre-job gate in `lint.yaml` and `labels.yaml` to short-circuit on the post-merge side too. After a version-bump PR (`bump-version` or `prepare-release`) merges to `main`, the resulting commit re-triggers every push-driven workflow with `head_ref = 'main'`, so the branch filter no longer applies. The new `VERSION_BUMP_COMMIT_PREFIXES` constant in `repomatic.binary` lists three stable commit-message prefixes (`Bump major version to `, `Bump minor version to `, `[changelog] Post-release bump `), and the gate now also checks `startsWith(github.event.head_commit.message || '', ...)` against each prefix. The post-merge `push` (and any downstream `workflow_call` propagating the same event) skips the entire job graph instead of running it for an effective no-op. A second conformance test (`test_metadata_gate_skips_version_bump_commits`) parses the embedded prefixes and pins them to the constant. Also removes the dead `VERSION_BUMP_COMMIT_PREFIX = "[changelog] Bump"` constant from `tests/test_workflows.py`: it claimed a prefix that the `bump-version` template never produces (`bump-my-version`'s default commit message is the bare `Bump $part version to ...` title without a `[changelog]` wrapper).
- Skip Nuitka binary builds in `release.yaml::compile-binaries` on `Bump (major|minor) version to ` merge commits. A new `MANUAL_VERSION_BUMP_COMMIT_PREFIXES` constant in `repomatic.binary` carves out the user-initiated subset of `VERSION_BUMP_COMMIT_PREFIXES`, and `Metadata.skip_binary_build` now checks the push event's head commit message against it. The `[changelog] Post-release bump ` prefix is deliberately *not* in this subset: the `prepare-release` merge bundles the post-release-bump commit with the actual release commit (`[changelog] Release vX.Y.Z`) in a single push, and skipping the binary build on the head commit would also wipe out the release binary. Eliminates ~12 job-hours of Nuitka compilation per manual major/minor bump merge to `main`.
- Skip `tests.yaml` on `Bump (major|minor) version to ` merges via a metadata-job `if:` gate matching the same `MANUAL_VERSION_BUMP_COMMIT_PREFIXES` subset. The same release-commit-coupling argument applies: `[changelog] Post-release bump ` stays out of the gate so the post-merge test of the release-frozen tree (the only safety net, since `prepare-release` PRs are excluded by `branches-ignore`) is preserved. New conformance test `test_tests_metadata_gate_skips_manual_bumps` pins the YAML literals to the constant and asserts the carve-out.
- Add `is_version_bump_commit` and `is_version_bump_event` metadata outputs alongside the existing `is_version_bump_branch`. `is_version_bump_commit` is `True` when the push event's head commit message matches any `VERSION_BUMP_COMMIT_PREFIXES` member; `is_version_bump_event` is the disjunction of branch-side and commit-side flags, providing a single boolean for downstream consumers that don't care which event type produced the bump.
- Add per-file-type `*_changed` boolean metadata outputs (`yaml_changed`, `zsh_changed`, `workflows_changed`) computed against the current event's commit range, with fallback to "repo contains any such file" when the commit range is unavailable (`workflow_dispatch`). `lint.yaml`'s `lint-yaml`, `lint-zsh`, `lint-github-actions`, and `lint-workflow-security` jobs now gate on these booleans instead of the repo-level `*_files` lists, so they skip on pushes/PRs that don't touch the relevant file type. `lint-types` (mypy) and `lint-secrets` keep the broader repo-level gate because mypy results depend on the whole codebase and secrets can hide in any file.
- Add a `paths-ignore` filter to `lint.yaml`'s `push: main` trigger covering image extensions (`*.png`, `*.jpg`, `*.jpeg`, `*.gif`, `*.svg`, `*.webp`). Pushes whose entire diff is image files no longer schedule a workflow run at all. The per-job `*_changed` gates above prevent each linter from running for unrelated changes once the workflow is scheduled; this trigger filter cuts the upfront scheduling cost when none of the linters would have done useful work.
- Add a positive `paths:` filter to `docs.yaml`'s `push: main` trigger restricting builds to changes in `docs/**`, `**/*.py`, `pyproject.toml`, `uv.lock`, `readme.md`, `changelog.md`, `citation.cff`, or the docs workflow itself. Autofix merges that don't touch documentation sources no longer rebuild and redeploy the Sphinx site.
