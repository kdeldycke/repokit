...
- Skip CI for automated version-bump PRs (`major-version-increment`, `minor-version-increment`, `prepare-release`) in `tests.yaml`, `lint.yaml`, and `labels.yaml` via a `github.head_ref` check in the `metadata` pre-job. The skip propagates through `needs: metadata` to every dependent job, covering `workflow_call` invocations that bypass the trigger filter. The trigger-level `pull_request.branches-ignore` listings are kept as documentation of intent but do not actually exclude these PRs: `branches-ignore` matches the PR's *base* branch (always `main`), not its head ([actions/runner#1591](https://github.com/actions/runner/issues/1591)). Collapses eight per-job `github.head_ref` guards in `lint.yaml` to one gate. `VERSION_BUMP_BRANCHES` in `repomatic.binary` is the single source of truth, enforced by `tests/test_workflows.py`.
- Extend the `metadata` gate in `lint.yaml`, `labels.yaml`, `tests.yaml`, and `release.yaml::compile-binaries` to also skip on the push side after a version-bump merge to `main`. The new `VERSION_BUMP_COMMIT_PREFIXES` constant lists commit-message prefixes (`Bump major version to `, `Bump minor version to `, `[changelog] Post-release bump `) checked via `startsWith` in the gate. The `[changelog] Post-release bump ` prefix is kept out of the Nuitka and test gates so the post-merge safety run is preserved.
...
- Add a `paths-ignore` filter covering image extensions to `lint.yaml`'s `push: main` trigger, and a positive `paths:` filter to `docs.yaml`'s `push: main` trigger restricting it to `docs/**`, `**/*.py`, `pyproject.toml`, `uv.lock`, `readme.md`, `changelog.md`, `citation.cff`, and the docs workflow itself. Both cut upfront scheduling cost when no relevant files changed.
